...
export function LanguageSwitcher() {
  ...
  const rest = isLocale(currentMaybeLocale) ? parts.slice(1) : parts;
  const base = `/${rest.join("/")}`;
  const current = isLocale(currentMaybeLocale) ? currentMaybeLocale : "pt";
...
function FlagIcon({ loc }: { loc: Locale }) {
  if (loc === "pt") {
    return (
